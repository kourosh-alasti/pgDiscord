...
function resolveTableName(input: string, schema: SchemaInfo): string | null {
  ...
  const matches = schema.tables.filter((t) => t.name.toLowerCase().includes(normalized));
  if (matches.length === 1) {
 ...
 */
export function interpretNaturalLanguage(input: string, schema: SchemaInfo): NlpResult | null {
  const text = input.trim();
  const lower = text.toLowerCase();

  // List all tables
  if (
    /^(?:list|show|get|display|what are)(?:\s+all)?(?:\s+the)?\s+tables?/i.test(lower) ||
