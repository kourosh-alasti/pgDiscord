...
import { ConnectionRegistry, maskConnectionString, validateConnectionString } from "../db/registry";
...
export const connectCommand = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("url")
        .setDescription("PostgreSQL URL (visible in channel — omit to use a private modal instead)")
        .setRequired(false),
    ),
...
async function handleConnect(
  ...
  const usedPublicOption =
    interaction.isChatInputCommand() && interaction.options.getString("url") !== null;
  ...
  try {
    ...
    const manager = await registry.connect(interaction.user.id, connectionString.trim());
